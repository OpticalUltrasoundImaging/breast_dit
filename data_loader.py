...
class pCRDataset(Dataset):
    ...
    def __init__(self,datatype, info_file, IDs,root_dir, cyc_num, transform=None,transform_US=None,transform_DOT=None):
        """
        Arguments:
        ...
        if datatype=='US' or datatype=='DOT':
            ...
            self.datatype = datatype
            self.IDs=IDs
            self.us_frame = pd.read_excel(info_file,sheet_name='Cyc'+cyc_num+'_US')
            self.dot_frame = pd.read_excel(info_file,sheet_name='Cyc'+cyc_num+'_DOT')
            self.us_num = []
            self.dot_num = []
            self.usdot_num=[0]
            for i in range(len(self.IDs)):
                ...
                #print(self.IDs[i],P_us_info['Pre_chemo'],P_us_info['Cyc'+cyc_num])
                if self.us_num==[]:
                    
                    self.us_num=[P_us_info['Pre_chemo'].values[0]*P_us_info['Cyc'+cyc_num].values[0]]
                    self.dot_num=[P_dot_info['Pre_chemo'].values[0]*P_dot_info['Cyc'+cyc_num].values[0]]
                    self.usdot_num+=[self.us_num[-1]*self.dot_num[-1]]
                else:
                    self.us_num.append(P_us_info['Pre_chemo'].values[0]*P_us_info['Cyc'+cyc_num].values[0])
                    self.dot_num.append(P_dot_info['Pre_chemo'].values[0]*P_dot_info['Cyc'+cyc_num].values[0])
    ...
    def __getitem__(self, idx):
        ...
        if self.datatype=='US':
            ...
            ## US Data
            us_prechemo_id = us_idx//us_line['Cyc'+self.cyc].values[0]
            us_cyc_id = us_idx%us_line['Cyc'+self.cyc].values[0]
            # locate folder path
            ...
            dot_prechemo_id = dot_idx//dot_line['Cyc'+self.cyc].values[0]
            dot_cyc_id = dot_idx%dot_line['Cyc'+self.cyc].values[0]
            # locate folder path
